...
public class FilterEventNameAdapter extends RecyclerView.Adapter<FilterEventNameAdapter.Holder> {
    ...
    public Holder onCreateViewHolder(@NonNull ViewGroup viewGroup, int i) {
        View view=mInflater.inflate(R.layout.dy_event_name_filter_item,viewGroup,false);
    ...
    public void onBindViewHolder(@NonNull Holder holder, int i) {
        if (mEventNameList.get(i).getChoose()==0){
            holder.mEventName.setBackground(mContext.getResources().getDrawable(R.drawable.dy_event_name_normal_bg));
        }else {
            holder.mEventName.setBackground(mContext.getResources().getDrawable(R.drawable.dy_event_name_select_item_bg));
        }
